    ...
    <main>
        ...
        <div>
            <div style="text-align: center;">
                <div class="citiestd13"> Party Ledger ({{ $party->party_type }}) </div>
                <table cellspacing="0" cellpadding="3">
                    <tr>
                        <td width="60%" class="supAddress">
                                <div><strong>Name : </strong>{{ $party->name . ' - ' . $party->code }}</div>
                                <div><strong>contact : </strong>{{ $party->contact }}</div>
                                <div><strong>Alternate Contact : </strong>{{ $party->alternate_contact }}</div>
                ...
                <table border="1" class="invoice-info" cellspacing="0" cellpadding="3">
                    ...
                    <tbody>
                        ...
                        @foreach ($partyLedger as $report)
                            ...
                            @if ($report->type == 'Payable')
                                @php
                                    ...
                                    $totalDr += $report->amount;
                                    $balance += $report->amount;
                                @endphp
                            @else
                                @php
                                    $crAmount = $report->amount;
                                    $totalCr += $report->amount;
                                    $balance -= $report->amount;
                                @endphp
                            @endif
                            <tr>
                                <td>{{ $i++ }}</td>
                                <td class="citiestd15">{{ $report->paymentDate }}</td>
                                <td class="text-center">{{ $report->type }} - {{ $report->purchase_no }} {{ $report->sale_no }}</td>
                                <td class="text-center">{{ $drAmount }}</td>
                        ...
                        <tr>
                            ...
                            <td class="text-center">{{ $totalDr }} </td>
                            <td class="text-center">{{ $totalCr }}</td>
                            <td class="text-center">{!! Session::get('companySettings')[0]['currency'] !!}.{{ $balance }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
