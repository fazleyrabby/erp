                                    ...
                                    <div class="col-md-2">
                                        <button type="button" id="checkOutCart"
                                            class="check-out-btn my_button float-right btn-block"><i
                                                class="fa fa-save"></i> <span class="check-out-text"> Place Order</span> 
                                        </button>
                                    </div>
                                </div>
                            </div>
...
@section('javascript')
    <script>
        ...
        function calculateTotal() {
            var totalAmount = 0;
            var i = 0;
            var advance_payment=$('#advance_payment_text').text();
            ...
            var grandTotal = parseFloat(totalAmount) + parseFloat(transport) + parseFloat(vat) + parseFloat(ait) -
                parseFloat(discount);
            var grandSum= grandTotal -  parseFloat(advance_payment);
            $("#grandTotal").text(grandTotal);
            $("#grandSum").text(grandSum);
        ...
        $("#checkOutCart").click(function(e) {
            ...
            var payment = parseFloat($("#payment").val())
            var current_payment = parseFloat($('#advance_payment_text').text()) + parseFloat($("#payment").val()); 
            ...
            if (isSave == "save") {
                ...
                if ((totalDue > leftCredit) && (saleType != 'walkin_sale')) {
                    Swal.fire({
            ...
            fd.append('SalesOrder', "SalesOrder");
            fd.append('_token', _token);
            clearErrorMessage();
            ...
            $.ajax({
                url: "{{ route('sale.service.completeOrderCheckOutCart') }}",
                method: "POST",
                data: fd,
                contentType: false,
                processData: false,
                ...
                success: function(result) {
                   ...
                   // alert(JSON.stringify(result));
                    ...
                    if (result.mismatch == "NotAvailable") {
                        Swal.fire('Error!', 'Error: Some product Quantity is  not available','error');
                        return 0;
