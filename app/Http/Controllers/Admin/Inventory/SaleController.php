        ...
        if (Session::get('sale_cart_array') != null) {
            ...
            foreach (Session::get('sale_cart_array') as $keys => $values) {
                ...
                $cart .= '<tr><td>'.$i++.'
                    ...
                    '<td class=""><div class="input-group input-group-sm"><button class="btn btn-outline-secondary qty-down" type="button" data-pid="'.$productId.'" data-wid="'.$warehouseId.'">−</button><input type="number" min="1" id="quantity_'.$productId.'_'.$warehouseId.'" name="quantity[]" class="form-control text-center" onblur="loadCartandUpdate('.$productId.','.$warehouseId.')" value="'.Session::get('sale_cart_array')[$keys]['product_quantity'].'" /><button class="btn btn-outline-secondary qty-up" type="button" data-pid="'.$productId.'" data-wid="'.$warehouseId.'">+</button></div>'.$btn.'</td>'.
